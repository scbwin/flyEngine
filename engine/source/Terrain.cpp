...
/*#if !USE_LOD_DISTANCE
  ...
  void Terrain::getTreeNodesForRendering(TreeNode * node, const glm::vec3 & cam_pos_model_space, std::vector<TreeNode*>& nodes, const glm::mat4& mvp)
  {
    ...
    if (error > _errorThreshold && node->_southWest) {
      getTreeNodesForRendering(node->_southWest, cam_pos_model_space, nodes, mvp);
      getTreeNodesForRendering(node->_southEast, cam_pos_model_space, nodes, mvp);
      getTreeNodesForRendering(node->_northWest, cam_pos_model_space, nodes, mvp);
